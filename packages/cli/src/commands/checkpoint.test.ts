...
import { initProjectMemory, listMemories } from 'pamh-core'
...
describe('checkpoint command', () => {
  ...
  beforeEach(async () => {
    ...
    tempDir = await mkdtemp(join(tmpdir(), 'pamh-cli-checkpoint-test-'))
  ...
  it('creates checkpoint memories through the CLI fallback command', async () => {
    ...
    await program.parseAsync(
      [
        ...
        '--fact',
        'AGENTS.md can call memory checkpoint when MCP tools are missing.',
        '--agent',
        'codex',
      ],
      { from: 'node' }
    ...
    expect(memories.map((memory) => memory.metadata.status)).toEqual([
      'proposed',
      'proposed',
      'proposed',
    ])
    ...
    expect(log).toHaveBeenCalledWith('Checkpoint recorded: 3 proposed memories')
